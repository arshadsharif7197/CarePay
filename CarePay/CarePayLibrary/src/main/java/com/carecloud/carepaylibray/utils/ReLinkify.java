...
/**
 * Created by lmenendez on 1/8/18
 */

public class ReLinkify {
    /**
    ...
    private static final void gatherLinks(ArrayList<LinkSpec> links,
                                          Spannable spannable, Pattern pattern, String[] schemes,
    ...
    private static final void gatherTelLinks(ArrayList<LinkSpec> links, Spannable spannable) {
        Pattern pattern = Patterns.PHONE;
        Matcher m = pattern.matcher(spannable);

        MatchFilter phoneMatcher = sPhoneNumberMatchFilter;

        while (m.find()) {
            int start = m.start();
            int end = m.end();

            if (phoneMatcher.acceptMatch(spannable, start, end)) {
                LinkSpec spec = new LinkSpec();
                String tel = m.group(0);

                spec.url = "tel:" + normalizeNumber(tel);
                spec.start = start;
                spec.end = end;
    ...
    private static final void gatherMapLinks(ArrayList<LinkSpec> links, Spannable spannable) {
